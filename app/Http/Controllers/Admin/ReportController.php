...
class ReportController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.report.layout', compact(['labels', 'customers', 'staffs', 'services', 'products', 'status','locations']));
    }
    ...
    public function earningTable(Request $request)
    {
        ...
        return \datatables()->of($bookings)
            ...
            ->editColumn('amount_to_pay', function ($row) {
                return currencyFormatter(number_format((float)$row->amount_to_pay, 2, '.', ''));
            })
        ...
        return \datatables()->of(collect($items))
            ...
            ->editColumn('customer_name', function ($row) {
                return '<a href="' . route('admin.customers.show', [$row->user->id]) . '" class=""
                    data-toggle="tooltip" data-original-title="'.__('app.clickToView').'"><i class="icon-user"></i> '. $row->user->name.'</a>';
            })
            ...
            ->editColumn('amount', function ($row) {
                ...
                if($status == 'completed'){
                    return '<i data-toggle="tooltip" data-original-title="'.__('app.payment').' '.__('app.done').'" style="color:green" class="fa fa-check-square"></i> '.currencyFormatter($row->amount_to_pay);
                }
                else{
                    return ' <i data-toggle="tooltip" data-original-title="'.__('app.payment').' '.__('app.pending').'" style="color:red" class="fa fa-times-circle"></i> '.currencyFormatter($row->amount_to_pay);
                }
            })
