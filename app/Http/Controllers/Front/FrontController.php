...
class FrontController extends Controller
{
    ...
    public function index()
    {
        ...
        $deals = $deals->where('start_date_time', '<=', Carbon::now()->setTimezone($this->settings->timezone))->where('end_date_time', '>=', Carbon::now()->setTimezone($this->settings->timezone))->latest()->take('6')->get();
        ...
        if (request()->ajax()) {
            if ($this->frontThemeSettings->front_theme == 'theme-2') {
                ...
                return Reply::dataOnly(['view' => $view, 'html' => $html, 'categories' => $categories, 'services' => $services, 'deals' => $deals, 'deal_count' => $deals->count()]);
            }
   ...
   /*     if ($this->frontThemeSettings->front_theme == 'theme-2') {
        ...
        return view('front.index', compact('categories', 'images', 'section_contents', 'customerFeedbacks', 'offers', 'products', 'content'));
    }
    ...
    public function manageBooking(Request $request)
    {
        ...
        return view('front-new.booking', compact('coupons', 'setting', 'credentials', 'frontThemeSetting', 'products', 'bookingDetails', 'couponData', 'tax', 'categories', 'locale', 'type'));
    }
    ...
    public function cartPage(Request $request, $url = null)
    {
      ...
      /*  if ($this->frontThemeSettings->front_theme == 'theme-2') {
        ...
        return view('front.cart_page', compact('products', 'bookingDetails', 'taxes', 'couponData', 'type'));
    }
    ...
    public function saveBooking(StoreFrontBooking $request)
    {
        ...
        if ($this->frontThemeSettings->front_theme == 'theme-2') {
            return response(Reply::successWithData(__('messages.front.success.bookingCreated'), ['action' => 'bookingCreated']))->withCookie(Cookie::forget('bookingDetails'))->withCookie(Cookie::forget('couponData'))->withCookie(Cookie::forget('products'));
        }
    ...
    public function serviceDetail(Request $request, $categorySlug, $serviceSlug)
    {
        ...
        if ($this->frontThemeSettings->front_theme == 'theme-2') {
            return view('front-new.service_detail', compact(['service', 'reqProduct', 'categories']));
        }
