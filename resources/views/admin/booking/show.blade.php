            ...
            <table class="table table-condensed">
                ...
                @foreach($booking->items as $key=>$item)
                    <tr>
                        ...
                        <small>@if(is_null($item->business_service_id)) @lang('app.product') @else @lang('app.service') @endif</small></td>
                        <td>{{ currencyFormatter(number_format((float)$item->unit_price, 2, '.', '')) }}</td>
                        <td>x{{ $item->quantity }}</td>
                        @if ($booking->deal_id!='')
                            <td class="text-right">{{ currencyFormatter(number_format((float)($item->unit_price  * $item->quantity), 2, '.', ''))}} x {{$booking->deal_quantity}} = {{ currencyFormatter(number_format((float)($item->unit_price  * $item->quantity * $booking->deal_quantity), 2, '.', ''))}}</td>
                        @else
            ...
            <div class="row">
                <div class="col-md-12">
                    <table class="table table-condensed">
                        ...
                        <tr class="h6">
                            <td class="border-top-0 text-right">@lang('app.productSubTotal')</td>
                        ...
                        <tr class="h5">
                            ...
                            <td>{{ currencyFormatter(number_format((float)$booking->amount_to_pay, 2, '.', '')) }}</td>
                        </tr>
                    </table>
                </div>
            </div>
        ...
        {{-- @if ($booking->status == 'completed' && !$user->is_admin)
            <div class="col-md-12 font-italic">
                <h4 class="text-info cust_feedback_msg_title @if ($booking->feedback == null) d-none @endif">
                    @lang('modules.booking.feedbackMessage')</h4>
                ...
                <p class="cust_feedback_msg @if ($booking->feedback == null) d-none @endif">
                    {{ $booking->feedback ? $booking->feedback->feedback_message : ''}}</p>
            </div>
        @endif --}}
