...
<table class="table table-bordered table-condensed" width="100%" id="deal_table">
    ...
    @foreach ($deal_items as $deal_item)
        <tr id="row{{$deal_item->businessService->id}}">
            ...
            <td><input type="hidden" class="deal-price-{{$deal_item->businessService->id}}" name="deal_unit_price[]" value="{{$deal_item->unit_price}}">{{currencyFormatter($deal_item->unit_price)}}</td>
            <td style="width: 15%">
            ...
            <td name="deal-subtotal[]" class="deal-subtotal-{{$deal_item->businessService->id}}">{{currencyFormatter($deal_item->unit_price*$deal_item->quantity)}} </td>
            <td style="width: 15%">
            ...
            <td name="deal-total[]" class="deal-total-{{$deal_item->businessService->id}}">{{currencyFormatter($deal_item->total_amount)}}</td>
    ...
    <tr>
        <td colspan="3"></td>
        <td id="deal-sub-total">{{currencyFormatter($deal_item->deal->original_amount)}}</td>
        <td id="deal-discount-total">{{currencyFormatter($deal_item->deal->original_amount-$deal_item->deal->deal_amount)}}</td>
        <td id="deal-total-price">{{currencyFormatter($deal_item->deal->deal_amount)}}</td>
    </tr>
</table>
